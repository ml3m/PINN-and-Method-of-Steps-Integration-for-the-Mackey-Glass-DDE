...
On each interval $[k\tau, (k+1)\tau]$ the delayed argument $t - \tau$ falls in $[(k-1)\tau, k\tau]$, where $x$ has already been computed (or, for $k = 0$, evaluated from the constant history). The result is a non-autonomous ODE segment:
...
Here, $\hat{x}(\cdot)$ is a continuous interpolant of the previously stored trajectory. Step 7 dispatches the local initial-value problem (IVP) to an embedded fourth/fifth-order Runge–Kutta pair [Dormand & Prince, 1980], which adapts its step size to maintain the specified error tolerances.
